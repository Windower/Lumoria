namespace Lumoria.Cli {
    ...
    public class SessionManagerService : Object {
        ...
        private class LaunchState {
            ...
            public LaunchState (int pid, string prefix_id, string wineserver_path, string[] env) {
                this.pid = pid;
        ...
        public uint launch (
            ...
            cancel_idle_exit ();
            launches[child_pid] = new LaunchState (child_pid, prefix_id, wineserver_path, env);

            var pid_copy = child_pid;
            ChildWatch.add ((Pid) child_pid, (pid, status) => {
                Process.close_pid (pid);
                on_wrap_exited (pid_copy);
            });
        ...
        public void stop (string prefix_id) {
            ...
            foreach (var state in states) stop_launch_state (state);
        }
        ...
        private void stop_launch_state (LaunchState state) {
            Posix.kill ((Posix.pid_t) state.pid, Posix.Signal.TERM);
            run_wineserver_kill (state);
        }

        private void run_wineserver_kill (LaunchState state) {
            var ws = state.wineserver_path;
            if (ws == "" || !FileUtils.test (ws, FileTest.EXISTS)) return;
            try {
                int status;
                Process.spawn_sync (null, { ws, "-k" }, state.env, SpawnFlags.SEARCH_PATH,
                    null, null, null, out status);
            } catch (Error e) {
                warning ("wineserver -k failed: %s", e.message);
            }
        }

        private void on_wrap_exited (int pid) {
            launches.unset (pid);
            if (launches.size == 0) {
                schedule_idle_exit ();
            }
        ...
        private string launch_response (Json.Object obj) throws Error {
            var pid = launch (
                obj.get_string_member ("prefix_id"),
    ...
    public int cmd_session_manager (string[] args) {
        if (args.length >= 3) {
            switch (args[2]) {
                case "stop-all":
                    return session_client_call (null);
                case "stop":
                    if (args.length < 4) {
                        stderr.printf ("Usage: lumoria session-manager stop <prefix-id>\n");
                        return 1;
                    }
                    return session_client_call (args[3]);
            }
        }
        return run_session_service ();
    }

    private int session_client_call (string? prefix_id) {
        try {
