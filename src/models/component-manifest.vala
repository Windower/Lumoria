namespace Lumoria.Models {

    public class ComponentManifest : InstallableManifest {
        public string github_repo { get; set; default = ""; }
        public string asset_regex { get; set; default = ""; }
        public string checksum_regex { get; set; default = ""; }
        public string default_version { get; set; default = "latest"; }
        public string source_archive { get; set; default = ""; }
        public Gee.ArrayList<string> installer_ids { get; owned set; default = new Gee.ArrayList<string> (); }
        public Gee.HashMap<string, string> system_env_defaults { get; owned set; default = new Gee.HashMap<string, string> (); }
        public Gee.HashMap<string, string> overrides { get; owned set; default = new Gee.HashMap<string, string> (); }

        public static ComponentManifest from_json (Json.Object obj) throws Error {
            var s = new ComponentManifest ();
            s.parse_installable (obj);
            s.github_repo = json_string (obj, "github_repo");
            s.asset_regex = json_string (obj, "asset_regex");
            s.checksum_regex = json_string (obj, "checksum_regex");
            s.default_version = json_string (obj, "default_version", "latest");
            s.source_archive = json_string (obj, "source_archive");
            s.installer_ids = json_string_array (obj, "installers");
            s.system_env_defaults = json_string_map (obj, "system_env_defaults");
            s.overrides = json_string_map (obj, "overrides");
            return s;
        }

        public bool supports_installer (string installer_id) {
            return installer_ids.size == 0 || installer_ids.contains (installer_id);
        }

        public static Gee.ArrayList<ComponentManifest> load_all_from_resource () throws Error {
            return load_named_manifests_from_resource<ComponentManifest> (
                "components",
                ManifestStore.list_ids ("components"),
                "component",
                (obj, _) => {
                    return ComponentManifest.from_json (obj);
                }
            );
        }
    }
}
