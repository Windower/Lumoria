namespace Lumoria.Utils {
    ...
    /*
     * libgit2-glib gives transfer callbacks no way to abort a running download, so a cancel
     * takes effect at the next phase boundary rather than mid-transfer.
     */
    public void git_clone (
        ...
        if (dot_git.query_exists ()) {
            ...
            try {
                ...
                throw new LumoriaError.FAILED ("git: clone failed: %s", e.message);
            }
            if (repo == null) {
                throw new LumoriaError.INTERNAL ("git: clone returned null repository for %s", url);
            }
        }
    ...
    public void git_pull (
        ...
        var dot_git = File.new_for_path (Path.build_filename (dest, ".git"));
        if (!dot_git.query_exists ()) {
            throw new LumoriaError.FAILED ("git: not a git working tree: %s", dest);
        }
    ...
    private Ggit.Repository open_existing_repo (File location) throws Error {
        try {
            ...
            if (repo == null) {
                throw new LumoriaError.INTERNAL ("git: failed to open repository at %s", location.get_path ());
            }
            return repo;
        } catch (Error e) {
            throw new LumoriaError.FAILED ("git: open failed: %s", e.message);
        }
    }
    ...
    private void fetch_origin (Ggit.Repository repo, owned GitProgressCallback? progress) throws Error {
        ...
        try {
            ...
            throw new LumoriaError.FAILED ("git: lookup_remote(origin) failed: %s", e.message);
        }
        if (remote == null) {
            throw new LumoriaError.FAILED ("git: no 'origin' remote configured");
        }
        ...
        try {
            ...
            throw new LumoriaError.FAILED ("git: fetch failed: %s", e.message);
        }
    }
    ...
    private void apply_target (Ggit.Repository repo, GitTarget target) throws Error {
        ...
        if (oid == null) {
            throw new LumoriaError.FAILED ("git: could not resolve target");
        }
        ...
        try {
            ...
            throw new LumoriaError.FAILED ("git: commit %s not reachable: %s", oid.to_string (), e.message);
        }
        if (commit_obj == null) {
            throw new LumoriaError.FAILED ("git: commit %s not found after fetch", oid.to_string ());
        }
        ...
        try {
            ...
            throw new LumoriaError.FAILED ("git: hard reset to %s failed: %s", oid.to_string (), e.message);
        }
    }
    ...
    private Ggit.OId? resolve_target_oid (Ggit.Repository repo, GitTarget target) throws Error {
        if (target.commit != "") {
            ...
            if (target.tag != "") {
                ...
                if (tag_oid == null) {
                    throw new LumoriaError.NOT_FOUND ("git: tag %s not found", target.tag);
                }
        ...
        if (target.branch != "") {
            ...
            if (oid == null) {
                throw new LumoriaError.NOT_FOUND ("git: branch origin/%s not found", target.branch);
            }
            return oid;
        }
